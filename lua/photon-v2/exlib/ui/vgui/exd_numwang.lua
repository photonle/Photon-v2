...
local function ScrollValue( wang, button, delta )
	...
	wang:SetValue( wang:GetValue() + (delta * self:GetInterval()) )
...
function PANEL:Init()
	...
	self.Up.OnMouseWheeled = function( button, delta ) 
		if (self:GetDisabled()) then return end
	...
	self.Down.OnMouseWheeled = function( button, delta )  
		if (self:GetDisabled()) then return end
		self:SetValue( self:GetValue() + (delta * self:GetInterval()) ) 
	end
...
function PANEL:Think()
	...
	if ( self.mouseAnchor ) then
		self:SetValue( self.valAnchor + ((self.mouseAnchor - gui.MouseY()) * self:GetInterval()) )
	end
...
derma.DefineControl( class, "EX Number Wang", PANEL, base )