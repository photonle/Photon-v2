print("[EX VGUI] exd_treenode.lua")

local class = "EXDTree_Node"
local base = "DPanel"
local PANEL = {}
...
function PANEL:Init()
	...
	self.Icon = vgui.Create( "EXDIcon", self )
...
function PANEL:PerformLayout()
	...
	self.ChildNodes:StretchToParent( 0, LineHeight, 0, 0 )
	self.ChildNodes:DockPadding(self:GetIndentSize(), 0, 0, 0)
...
function PANEL:AddNode( strName, strIcon )
	...
	local pNode = vgui.Create( "EXDTree_Node", self )
	pNode.NodeLevel = self.NodeLevel + 1
...
function PANEL:Copy()
	...
	local copy = vgui.Create( "EXDTree_Node", self:GetParent() )
