...
function PANEL:Init()
	if IsValid(self:GetParent()) then
		self:SetSkin(self:GetParent():GetSkin().ID)
	end
...
function PANEL:OpenAutoComplete( tab )
	...
	local this = self
	self.Menu = DermaMenu()
	self.Menu:SetSkin(self:GetSkin().ID)
	self.menuOpen = true
	for k, v in pairs( tab ) do
		...
		local o = self.Menu:AddOption( v, clickFunction)

		local oldPressed = o.OnMousePressed
		
		function o:OnMousePressed()
			print("mouse pressed")

			-- this.menuOpen = false
			clickFunction()
			-- this:SetText( v ) 
			-- this:SetCaretPos( v:len() ) 
			-- this:RequestFocus() 
			-- o:DoClickInternal()
			-- return true
			oldPressed(o)
	...
	self.Menu:SetFocusTopLevel(true)
...
function PANEL:OnFocusChanged() end
...
derma.DefineControl( class, "EX Text Entry", PANEL, base )
